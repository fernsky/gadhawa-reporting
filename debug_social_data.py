...
def debug_social_data():
    ...
    if toilet_count > 0:
        ...
        for record in WardWiseToiletType.objects.all()[:5]:
            print(f"     Ward {record.ward_number}: {record.toilet_type} = {record.households} households")
    ...
    try:
        ...
        formatted_muni = processor._format_municipality_data_for_pie_chart(data.get('municipality_data', {}))
        print(f"   Formatted municipality data: {formatted_muni}")
        
        # Test ward data formatting
        formatted_ward = processor._format_ward_data_for_bar_chart(data.get('ward_data', {}))
        print(f"   Formatted ward data keys: {list(formatted_ward.keys())}")
        for ward_key, ward_data in formatted_ward.items():
            print(f"     Ward {ward_key}: {ward_data.get('total_population', 0)} total, {len(ward_data.get('demographics', {}))} demographics")
    except Exception as e:
        print(f"   Error: {e}")
