...
class SolidWasteManagementProcessor(BaseSocialProcessor):
    ...
    def get_data(self):
        ...
        for waste_choice in SolidWasteManagementChoice.choices:
            ...
            waste_households = WardWiseSolidWasteManagement.objects.filter(
                solid_waste_management=waste_code
            ).aggregate(total=models.Sum('households'))['total'] or 0
            
            if waste_households > 0:  # Only include waste management types with households
        ...
        for waste_code in waste_management_data:
            if total_households > 0:
                waste_management_data[waste_code]['percentage'] = (
                    waste_management_data[waste_code]['population'] / total_households * 100
                )
        
        # Ward-wise data
        ward_data = {}
        for ward_num in range(1, 8):  # Wards 1-7
            ward_households = WardWiseSolidWasteManagement.objects.filter(
            ...
            if ward_households > 0:
                ...
                for waste_choice in SolidWasteManagementChoice.choices:
                    ...
                    waste_households_ward = WardWiseSolidWasteManagement.objects.filter(
                        ward_number=ward_num, solid_waste_management=waste_code
                    ).aggregate(total=models.Sum('households'))['total'] or 0
                    
                    if waste_households_ward > 0:
                        ward_data[ward_num]['waste_management_types'][waste_code] = {
                            'name_nepali': waste_name,
                            'population': waste_households_ward,
                            'percentage': (waste_households_ward / ward_households * 100) if ward_households > 0 else 0
                        }
    ...
    def generate_analysis_text(self, data):
        ...
        analysis_parts.append(
            f"लुङ्ग्री गाउँपालिकामा कुल {format_nepali_number(total_households)} घरपरिवारको ठोस फोहोर व्यवस्थापनको अवस्था विश्लेषण गर्दा विभिन्न विधिहरूको प्रयोग भइरहेको देखिन्छ।"
        )
        
        # Environmental impact analysis
        if municipality_data:
            # Find the most common waste management method
            dominant_method = max(municipality_data.items(), key=lambda x: x[1]['population'])
            analysis_parts.append(
            ...
            eco_friendly_percentage = (eco_friendly_households / total_households * 100) if total_households > 0 else 0
        ...
        if ward_data and len(ward_data) > 1:
            ...
            for ward_num, ward_info in ward_data.items():
                ...
                eco_friendly_count = sum(
                    ward_info['waste_management_types'].get(method, {}).get('population', 0)
                    for method in ['COMPOST_MANURE', 'HOME_COLLECTION', 'WASTE_COLLECTING_PLACE']
                )
                eco_percentage = (eco_friendly_count / ward_info['total_population'] * 100) if ward_info['total_population'] > 0 else 0
        ...
        if 'COMPOST_MANURE' in municipality_data and municipality_data['COMPOST_MANURE']['population'] > 0:
            analysis_parts.append(
