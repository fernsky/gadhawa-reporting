...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f'Testing category: {category.name} / {category.name_nepali}')
            ...
            for i, section in enumerate(sections, 1):
                ...
                self.stdout.write(f'  Section number: {section.section_number} / {nepali_digits(section.section_number)}')
                ...
                if figures:
                    ...
                    for fig in figures:
                        self.stdout.write(f'    - Figure {fig.figure_number} / {nepali_digits(fig.figure_number)}: {fig.title}')
                ...
                if tables:
                    ...
                    for table in tables:
                        self.stdout.write(f'    - Table {table.table_number} / {nepali_digits(table.table_number)}: {table.title}')
            ...
            self.stdout.write(f'  category.name_nepali|default:category.name = {category.name_nepali or category.name}')
            self.stdout.write(f'  category.description_nepali|default:category.description = {category.description_nepali or category.description}')
            ...
            for section in sections[:3]:  # Test first 3 sections
                ...
                self.stdout.write(f'  section.title_nepali|default:section.title = {title_display}')
                self.stdout.write(f'  section.summary_nepali|default:section.summary = {desc_display}')
