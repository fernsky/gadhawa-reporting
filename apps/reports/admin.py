...
class ReportCategoryAdmin(admin.ModelAdmin):
    list_display = ['name', 'name_nepali', 'order', 'sections_count', 'is_active', 'created_at']
    ...
    def sections_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:reports_reportsection_changelist') + f'?category__id__exact={obj.id}'
...
class ReportSectionAdmin(admin.ModelAdmin):
    list_display = [
        'section_number', 'title', 'category', 'is_published', 
        'figures_count', 'tables_count', 'author', 'updated_at'
    ]
    ...
    def figures_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:reports_reportfigure_changelist') + f'?section__id__exact={obj.id}'
    ...
    def tables_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:reports_reporttable_changelist') + f'?section__id__exact={obj.id}'
...
class ReportFigureAdmin(admin.ModelAdmin):
    list_display = [
        'figure_number', 'title', 'section', 'figure_type', 
        'image_preview', 'order', 'created_at'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('section', 'figure_number', 'title', 'title_nepali', 'figure_type')
        }),
        ('Content', {
...
class ReportTableAdmin(admin.ModelAdmin):
    list_display = [
        'table_number', 'title', 'section', 'data_preview', 
        'order', 'created_at'
    ]
...
class PublicationSettingsAdmin(admin.ModelAdmin):
    list_display = [
        'municipality_name', 'report_title', 'version', 
...
class ReportDownloadAdmin(admin.ModelAdmin):
    list_display = [
        'download_type', 'section', 'ip_address', 
        'downloaded_at'
    ]
...
admin.site.site_title = "Gadhawa Admin"
