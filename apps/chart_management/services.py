...
class ChartGenerationService:
    ...
    def _get_existing_chart(
        self, chart_key: str, content_hash: str
    ) -> Optional[ChartCache]:
        ...
        try:
            if chart_type == ChartTypeChoice.PIE:
                return self.svg_generator.generate_pie_chart(
                    data=data, title=title, width=width, height=height, **kwargs
                )
    ...
    def _generate_png_file(
        self, chart_cache: ChartCache, svg_path: Path
    ) -> Optional[Path]:
    ...
    def cleanup_old_charts(self, days_old: int = 30) -> int:
        ...
        old_charts = ChartCache.objects.filter(
            created_at__lt=cutoff_date, access_count__lt=5  # Haven't been accessed much
        )
    ...
    def get_chart_stats(self) -> Dict[str, Any]:
        ...
        cache_hits = (
            total_charts
            - ChartGenerationLog.objects.filter(operation_type="svg_generation").count()
        )
